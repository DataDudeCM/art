// artist.js - A more complex brush painting library inspired by real-world techniques
//
// Includes:
// -BrushArtist
// -sketchyPaintedLine - A function that uses the BrushArtist to create sketchy lines with multiple passes and jitter for texture
// -sketchLine - A simpler function that creates sketchy lines using basic p5.js drawing functions, included for comparison and inspiration for the multi-pass logic in sketchyPaintedLine
...
function sketchyPaintedLine(x1, y1, x2, y2) {
  ...
  for (let i = 0; i < numStrokes; i++) {
    ...
    for (let j = 1; j < points.length; j++) {
        painter.paintStroke(points[j].x, points[j].y, {
          px: points[j-1].x,
          py: points[j-1].y,
...
function pointFromAngle(x, y, distance, angle) {
  // create a starting vector
  let start = createVector(x, y);
  
  // create an offset vector in the given direction and length
  let offset = p5.Vector.fromAngle(angle).mult(distance);
  
  // add offset to starting point
  let newPoint = p5.Vector.add(start, offset);
