...
function setup() {
    ...
    for (let j = 0; j < numcenters; j++) {
        ...
        for (let i = 0; i < numcracks; i++) {
            cracks.push(new Crack(xpos,ypos, random(TWO_PI), maxlife));
        }
    }
    //noLoop;
}
...
class Crack {
  constructor(x, y, angle, lifetime = maxlife *.4) {
    this.x = x;
    this.y = y;
    this.angle = angle;
  ...
  grow() {
    ...
    // Draw the crack
    stroke(0);
    strokeWeight(map(this.lifetime, maxlife, 0, 3, 0.5)); // Tapering effect
