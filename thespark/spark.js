/*
    THE SPARK
    Generative artwork by Chris + ChatGPT

    One ignition moves through a dormant network.
    Nodes amplify, weaken, split, or mutate the energy.

    CONTROLS
    --------------------------------------------------
    mouse click : ignite a spark
    SPACE       : rebuild network
    S           : save image
*/

let nodes = [];
let pulses = [];

let paintLayer;

let NODE_COUNT = 450;
let CONNECTION_RADIUS = 125;

let fadeAmount = 5;

let sparkTimer = 0;
let nextSparkTime = 60;

let MAX_NODES = 850;

let GROWTH_CHANCE = 0.16;
let GROWTH_MIN_DISTANCE = 45;
let GROWTH_MAX_DISTANCE = 120;

let focalPoints = [];
let voidPoints = [];

let palettes = [];
let currentPalette;
let backgroundColor;

function setup() {

  createCanvas(windowWidth, windowHeight);

  colorMode(HSB, 360, 100, 100, 100);
  strokeCap(ROUND);

  // Permanent artwork layer
  paintLayer = createGraphics(width, height);

  paintLayer.colorMode(
    HSB,
    360,
    100,
    100,
    100
  );

  paintLayer.strokeCap(ROUND);

  setupPalettes();
  choosePalette();

  buildComposition();
  buildNetwork();


  background(225, 30, 4);

}


// ----------------------------------------------------
// DRAW
// ----------------------------------------------------

function draw() {

  // Clear only the animation screen
  background(
    backgroundColor[0],
    backgroundColor[1],
    backgroundColor[2]
  );

  // Redisplay everything painted so far
  image(paintLayer, 0, 0);

  updateNodes();
  updatePulses();


  if (pulses.length === 0) {

    sparkTimer++;

    if (sparkTimer > nextSparkTime) {

      igniteRandomSpark();

      sparkTimer = 0;

      nextSparkTime = random(45, 120);
    }

  } else {

    sparkTimer = 0;

  }
}

function setupPalettes() {

  palettes = [

    // Earth + ember
    {
      name: "earth-ember",
      bg: [22, 18, 10],
      colors: [
        [18, 70, 78],   // rust
        [32, 72, 88],   // ochre
        [45, 55, 92],   // muted gold
        [188, 38, 68],  // dusty teal
        [12, 35, 58]    // warm brown-red
      ]
    },

    // Storm + signal
    {
      name: "storm-signal",
      bg: [220, 28, 10],
      colors: [
        [210, 45, 82],  // steel blue
        [198, 38, 95],  // pale cyan
        [228, 30, 62],  // slate
        [36, 80, 96],   // amber accent
        [245, 18, 80]   // gray-violet
      ]
    },

    // Botanical dusk
    {
      name: "botanical-dusk",
      bg: [160, 18, 8],
      colors: [
        [145, 42, 58],  // moss
        [168, 35, 70],  // turquoise green
        [92, 45, 62],   // olive
        [40, 18, 94],   // bone / cream
        [182, 22, 78]   // subdued aqua
      ]
    },

    // Faded heat
    {
      name: "faded-heat",
      bg: [8, 26, 10],
      colors: [
        [8, 62, 72],    // maroon-red
        [18, 68, 88],   // terracotta
        [28, 52, 92],   // sienna-peach
        [345, 26, 84],  // dusty rose
        [278, 18, 80]   // muted lilac
      ]
    }

  ];
}

function choosePalette() {

  currentPalette = random(palettes);

  backgroundColor = currentPalette.bg;
}

function getPaletteColor() {
  return random(currentPalette.colors);
}

function getRelatedPaletteColor(baseHue) {

  let best = currentPalette.colors[0];
  let bestDiff = 9999;

  for (let c of currentPalette.colors) {
    let diff = abs(((c[0] - baseHue + 540) % 360) - 180);
    if (diff < bestDiff) {
      bestDiff = diff;
      best = c;
    }
  }

  // Sometimes stay close, sometimes hop to another palette color
  if (random() < 0.65) {
    return [
      (best[0] + random(-8, 8) + 360) % 360,
      constrain(best[1] + random(-8, 8), 0, 100),
      constrain(best[2] + random(-8, 8), 0, 100)
    ];
  } else {
    let jump = random(currentPalette.colors);
    return [...jump];
  }
}

function buildComposition() {

  focalPoints = [];
  voidPoints = [];


  // MAIN FOCAL AREA
  // Keep it away from dead center.
  let mainX = random(width * 0.28, width * 0.72);
  let mainY = random(height * 0.25, height * 0.70);

  focalPoints.push({
    x: mainX,
    y: mainY,
    radius: min(width, height) * 0.32,
    strength: 1.0
  });


  // SECONDARY FOCAL AREA
  focalPoints.push({
    x: constrain(
      mainX + random(-width * 0.35, width * 0.35),
      width * 0.12,
      width * 0.88
    ),

    y: constrain(
      mainY + random(-height * 0.30, height * 0.30),
      height * 0.12,
      height * 0.88
    ),

    radius: min(width, height) * 0.22,
    strength: 0.55
  });


  // SMALL ACCENT AREA
  focalPoints.push({
    x: random(width * 0.1, width * 0.9),
    y: random(height * 0.1, height * 0.9),

    radius: min(width, height) * 0.15,
    strength: 0.28
  });


  // QUIET REGION / NEGATIVE SPACE
  //
  // Put it loosely opposite the main focal area.
  voidPoints.push({
    x: width - mainX,
    y: height - mainY,

    radius: min(width, height) * 0.30,
    strength: 0.9
  });
}

// ----------------------------------------------------
// NETWORK
// ----------------------------------------------------

function buildNetwork() {

  nodes = [];
  pulses = [];

  for (let i = 0; i < NODE_COUNT; i++) {

    let x = random(width * 0.07, width * 0.93);
    let y = random(height * 0.07, height * 0.93);

    nodes.push(new SparkNode(x, y));

  }

  // build neighbor relationships
  for (let a of nodes) {

    let candidates = [];

    for (let b of nodes) {

      if (a === b) continue;

      let d = dist(a.x, a.y, b.x, b.y);

      if (d < CONNECTION_RADIUS) {
        candidates.push({
          node: b,
          distance: d
        });
      }
    }

    // closest nodes first
    candidates.sort((p1, p2) => p1.distance - p2.distance);

    // each node gets only a few connections
    let connectionCount = floor(random(2, 5));

    for (
      let i = 0;
      i < min(connectionCount, candidates.length);
      i++
    ) {
      a.neighbors.push(candidates[i].node);
    }
  }

}


// ----------------------------------------------------
// NODE
// ----------------------------------------------------

class SparkNode {

  constructor(x, y) {

    this.x = x;
    this.y = y;

    this.neighbors = [];

    this.activation = 0;

    this.lastActivated = -1000;

    let r = random();

    if (r < 0.05) {
      this.type = "amplifier";
    }
    else if (r < 0.10) {
      this.type = "splitter";
    }
    else if (r < 0.15) {
      this.type = "damper";
    }
    else if (r < 0.20) {
      this.type = "mutator";
    }
    else {
      this.type = "normal";
    }

  }


activate(energy, col) {

  // Prevent the spark from immediately cycling backward
  if (frameCount - this.lastActivated < 22) {
    return;
  }

  this.lastActivated = frameCount;

  this.activation = constrain(energy, 0, 1);

  let newEnergy = energy;
  let newColor = [...col]


  // ---------------------------------------------
  // NODE PERSONALITIES
  // ---------------------------------------------

  if (this.type === "amplifier") {
    newEnergy *= 1.25;
    newColor[1] = constrain(newColor[1] + random(4, 10), 0, 100);
    newColor[2] = constrain(newColor[2] + random(4, 10), 0, 100);
  }


  if (this.type === "damper") {
    newEnergy *= 0.72;
    newColor[1] = constrain(newColor[1] - random(6, 12), 0, 100);
    newColor[2] = constrain(newColor[2] - random(8, 16), 0, 100);
  }


  if (this.type === "mutator") {
    newColor = getRelatedPaletteColor(newColor[0]);
  }


  newColor[0] = (newColor[0] + 360) % 360;


  // Let extremely weak energy survive longer
  if (newEnergy < 0.035) return;


  let composition =
  compositionValue(
    this.x,
    this.y
  );

  let options = this.neighbors.filter(n =>
    frameCount - n.lastActivated > 22
  );


  if (options.length === 0) return;


  shuffle(options, true);


  // ---------------------------------------------
  // BRANCHING
  // ---------------------------------------------

  let branches = 1;


  // Healthy sparks frequently branch
  let branchChance =
    newEnergy *
    map(
      composition,
      0,
      1,
      0.35,
      1.15
    );


  if (random() < branchChance) {
    branches++;
  }


  // Occasionally create another branch
  if (
    random() <
    newEnergy *
    map(
      composition,
      0,
      1,
      0.12,
      0.55
    )
  ) {

    branches++;
  }

  // Splitter nodes go nuts
  if (this.type === "splitter") {

    branches += floor(random(2, 4));

  }


  branches = min(
    branches,
    options.length
  );


  // ---------------------------------------------
  // PROPAGATE
  // ---------------------------------------------

  for (let i = 0; i < branches; i++) {

    let target = options[i];


    // Energy now survives much longer
    let loss = random(0.84, 0.98);


    pulses.push(

      new Pulse(
        this,
        target,
        newEnergy * loss,
        newColor
      )

    );

  }
  // ------------------------------------------------
  // OCCASIONALLY GROW INTO EMPTY SPACE
  // ------------------------------------------------

  let localComposition =
    compositionValue(
      this.x,
      this.y
    );


  let localGrowthChance =
    GROWTH_CHANCE *
    map(
      localComposition,
      0,
      1,
      0.35,
      1.8
    );


  if (
    newEnergy > 0.35 &&
    nodes.length < MAX_NODES &&
    random() < localGrowthChance
  ) {

    growNewNode(
      this,
      newEnergy,
      newColor
    );
  }

}


  update() {

    this.activation *= 0.94;

  }


  display() {

    if (this.activation <= 0.01) return;

    let size = 2 + this.activation * 12;

    let hue;

    switch (this.type) {

      case "amplifier":
        hue = 45;
        break;

      case "splitter":
        hue = 195;
        break;

      case "damper":
        hue = 0;
        break;

      case "mutator":
        hue = 285;
        break;

      default:
        hue = 35;
    }


    noStroke();


    // glow
    fill(
      hue,
      80,
      100,
      this.activation * 12
    );

    circle(
      this.x,
      this.y,
      size * 4
    );


    fill(
      hue,
      35,
      100,
      this.activation * 80
    );

    circle(
      this.x,
      this.y,
      size
    );

  }

}


// ----------------------------------------------------
// ENERGY PULSE
// ----------------------------------------------------

class Pulse {

  constructor(start, target, energy, col) {

    this.start = start;
    this.target = target;

    this.energy = energy;

    this.color = [...col];
    this.hueDrift = random(-0.25, 0.25);

    this.progress = 0;
    this.previousProgress = 0;

    this.speed = random(0.018, 0.045);

    this.dead = false;

    // Bend the trajectory slightly
    let mx = (start.x + target.x) / 2;
    let my = (start.y + target.y) / 2;

    let angle = atan2(
      target.y - start.y,
      target.x - start.x
    );

    let bend = random(-30, 30);

    this.cx =
      mx +
      cos(angle + HALF_PI) * bend;

    this.cy =
      my +
      sin(angle + HALF_PI) * bend;
  }


  update() {

    this.previousProgress = this.progress;

    this.progress += this.speed;
    this.color[0] = (this.color[0] + this.hueDrift + 360) % 360;


    if (this.progress >= 1) {

      this.progress = 1;

      this.paintArrival();

      this.target.activate(
        this.energy,
        this.color
      );

      this.dead = true;
    }
  }


  display() {

    let t1 = this.previousProgress;
    let t2 = this.progress;


    let px =
      quadraticPoint(
        this.start.x,
        this.cx,
        this.target.x,
        t1
      );


    let py =
      quadraticPoint(
        this.start.y,
        this.cy,
        this.target.y,
        t1
      );


    let x =
      quadraticPoint(
        this.start.x,
        this.cx,
        this.target.x,
        t2
      );


    let y =
      quadraticPoint(
        this.start.y,
        this.cy,
        this.target.y,
        t2
      );


    // Permanently paint this section
    this.paintStroke(px, py, x, y);


    // -----------------------------------------
    // LIVE ENERGY GLOW
    // -----------------------------------------

    stroke(
      this.color[0], this.color[1], this.color[2],
      10 * this.energy
    );

    strokeWeight(
      12 * this.energy
    );

    line(px, py, x, y);


    stroke(
      this.color[0], this.color[1], this.color[2],
      35 * this.energy
    );

    strokeWeight(
      4 * this.energy
    );

    line(px, py, x, y);


    stroke(
      this.color[0], this.color[1], this.color[2],
      95
    );

    strokeWeight(
      max(0.7, this.energy * 1.4)
    );

    line(px, py, x, y);


    noStroke();

    fill(
      this.color[0], this.color[1], this.color[2],
      100
    );

    circle(
      x,
      y,
      2 + this.energy * 5
    );
  }


  // ------------------------------------------------
  // BRUSH LANGUAGE
  // ------------------------------------------------

  paintStroke(x1, y1, x2, y2) {

    let brush = this.target.type;


    // -------------------------------------
    // NORMAL
    // fine translucent filament
    // -------------------------------------

    if (brush === "normal") {

      paintLayer.stroke(
        this.color[0], this.color[1], this.color[2],
        20
      );

      paintLayer.strokeWeight(
        0.6 + this.energy * 1.4
      );

      paintLayer.line(
        x1,
        y1,
        x2,
        y2
      );


      // subtle fuzzy duplicate

      paintLayer.stroke(
        this.color[0], this.color[1], this.color[2],
        5
      );

      paintLayer.strokeWeight(
        5
      );

      paintLayer.line(
        x1 + random(-1, 1),
        y1 + random(-1, 1),
        x2 + random(-1, 1),
        y2 + random(-1, 1)
      );
    }


    // -------------------------------------
    // AMPLIFIER
    // thicker energetic paint
    // -------------------------------------

    else if (brush === "amplifier") {

      paintLayer.stroke(
        this.color[0], this.color[1], this.color[2],
        8
      );

      paintLayer.strokeWeight(
        10 + this.energy * 8
      );

      paintLayer.line(
        x1,
        y1,
        x2,
        y2
      );


      paintLayer.stroke(
        this.color[0], this.color[1], this.color[2],
        30
      );

      paintLayer.strokeWeight(
        2 + this.energy * 3
      );

      paintLayer.line(
        x1,
        y1,
        x2,
        y2
      );
    }


    // -------------------------------------
    // DAMPER
    // broken dry-brush texture
    // -------------------------------------

    else if (brush === "damper") {

      let steps = 4;

      for (let i = 0; i < steps; i++) {

        if (random() < 0.65) {

          let t = random();

          let x = lerp(x1, x2, t);
          let y = lerp(y1, y2, t);

          paintLayer.noStroke();

          paintLayer.fill(
            this.color[0], this.color[1], this.color[2],
            random(5, 18)
          );

          paintLayer.circle(
            x + random(-3, 3),
            y + random(-3, 3),
            random(1, 4)
          );
        }
      }
    }


    // -------------------------------------
    // SPLITTER
    // several fine parallel filaments
    // -------------------------------------

    else if (brush === "splitter") {

      for (let i = 0; i < 3; i++) {

        let offset = random(-2.5, 2.5);

        paintLayer.stroke(
          this.color[0] + random(-8, 8),
          this.color[1] + random(-8, 8),
          this.color[2] + random(-8, 8),
          14
        );

        paintLayer.strokeWeight(
          random(0.4, 1.2)
        );

        paintLayer.line(
          x1 + offset,
          y1 + offset,
          x2 + offset,
          y2 + offset
        );
      }
    }


    // -------------------------------------
    // MUTATOR
    // richer chromatic stroke
    // -------------------------------------

    else if (brush === "mutator") {

      let secondHue =
        (this.color[0] + random(35, 100)) % 360;


      paintLayer.stroke(
        this.color[0],
        this.color[1],
        this.color[2],
        18
      );

      paintLayer.strokeWeight(
        3
      );

      paintLayer.line(
        x1,
        y1,
        x2,
        y2
      );


      paintLayer.stroke(
        secondHue,
        75,
        100,
        12
      );

      paintLayer.strokeWeight(
        7
      );

      paintLayer.line(
        x1 + random(-2, 2),
        y1 + random(-2, 2),
        x2 + random(-2, 2),
        y2 + random(-2, 2)
      );
    }
  }


  // ------------------------------------------------
  // MARK MADE WHEN A NODE IS REACHED
  // ------------------------------------------------

  paintArrival() {

    let x = this.target.x;
    let y = this.target.y;


    if (this.target.type === "amplifier") {

      paintLayer.noStroke();

      for (let r = 30; r > 2; r -= 5) {

        paintLayer.fill(
          this.color[0], this.color[1], this.color[2],
          2.5
        );

        paintLayer.circle(
          x,
          y,
          r * this.energy
        );
      }
    }


    else if (this.target.type === "splitter") {

      paintLayer.noStroke();

      for (let i = 0; i < 12; i++) {

        let angle = random(TAU);

        let d = random(3, 15);

        paintLayer.fill(
          this.color[0], this.color[1], this.color[2],
          random(8, 25)
        );

        paintLayer.circle(
          x + cos(angle) * d,
          y + sin(angle) * d,
          random(0.5, 2.5)
        );
      }
    }


    else if (this.target.type === "mutator") {

      paintLayer.noFill();

      paintLayer.stroke(
        this.color[0], this.color[1], this.color[2],
        20
      );

      paintLayer.strokeWeight(1);

      paintLayer.circle(
        x,
        y,
        random(8, 25)
      );
    }


    else if (this.target.type === "damper") {

      paintLayer.noStroke();

      paintLayer.fill(
        this.color[0], this.color[1], this.color[2],
        12
      );

      paintLayer.circle(
        x,
        y,
        random(3, 8)
      );
    }
  }
}


// ----------------------------------------------------
// UPDATE
// ----------------------------------------------------

function updatePulses() {

  for (let i = pulses.length - 1; i >= 0; i--) {

    pulses[i].update();

    pulses[i].display();

    if (pulses[i].dead) {
      pulses.splice(i, 1);
    }

  }

}


function updateNodes() {

  for (let node of nodes) {

    node.update();

    node.display();

  }

}


function compositionVector(x, y) {

  let vx = 0;
  let vy = 0;


  // Attraction toward focal areas
  for (let p of focalPoints) {

    let dx = p.x - x;
    let dy = p.y - y;

    let d =
      max(
        1,
        sqrt(
          dx * dx +
          dy * dy
        )
      );


    let influence =
      Math.exp(
        -sq(d / p.radius)
      ) *
      p.strength;


    vx +=
      (dx / d) *
      influence;

    vy +=
      (dy / d) *
      influence;
  }


  // Repulsion away from void areas
  for (let p of voidPoints) {

    let dx = x - p.x;
    let dy = y - p.y;

    let d =
      max(
        1,
        sqrt(
          dx * dx +
          dy * dy
        )
      );


    let influence =
      Math.exp(
        -sq(d / p.radius)
      ) *
      p.strength;


    vx +=
      (dx / d) *
      influence;

    vy +=
      (dy / d) *
      influence;
  }


  return createVector(
    vx,
    vy
  );
}

// ----------------------------------------------------
// SUBTLE DORMANT STRUCTURE
// ----------------------------------------------------

function drawDormantNetwork() {

  strokeWeight(0.5);

  for (let node of nodes) {

    for (let neighbor of node.neighbors) {

      let d = dist(
        node.x,
        node.y,
        neighbor.x,
        neighbor.y
      );

      let alpha = map(
        d,
        0,
        CONNECTION_RADIUS,
        3,
        0
      );

      stroke(
        210,
        20,
        60,
        alpha
      );

      line(
        node.x,
        node.y,
        neighbor.x,
        neighbor.y
      );

    }

  }

}


// ----------------------------------------------------
// IGNITION
// ----------------------------------------------------

function igniteRandomSpark() {

  let node = random(nodes);

  let c = getPaletteColor();
  node.activate(random(0.8, 1), c);

  node.activation = 1;

  node.activate(
    random(0.8, 1),
    hue
  );

}


function igniteAt(x, y) {

  let closest = null;
  let bestDistance = Infinity;

  for (let node of nodes) {

    let d = dist(
      x,
      y,
      node.x,
      node.y
    );

    if (d < bestDistance) {

      bestDistance = d;
      closest = node;

    }

  }


  if (closest) {

    closest.lastActivated = -1000;

    closest.activate(
      1,
      random(20, 210)
    );

  }

}

function lerpAngle(a, b, amount) {

  let difference =
    atan2(
      sin(b - a),
      cos(b - a)
    );

  return a +
    difference *
    amount;
}

function growNewNode(source, energy, hue) {

  // Noise gives growth some coherent directional tendency
  let noiseAngle =
    noise(
      source.x * 0.003,
      source.y * 0.003,
      frameCount * 0.002
    ) * TAU * 2;


  // Invisible compositional force
  let field =
    compositionVector(
      source.x,
      source.y
    );


  let angle = noiseAngle;


  if (field.mag() > 0.01) {

    let fieldAngle =
      field.heading();


    angle =
      lerpAngle(
        noiseAngle,
        fieldAngle,
        0.32
      );
  }


  // Keep randomness
  angle += random(
    -0.55,
    0.55
  );


  let distance =
    random(
      GROWTH_MIN_DISTANCE,
      GROWTH_MAX_DISTANCE
    );


  let x =
    source.x +
    cos(angle) * distance;


  let y =
    source.y +
    sin(angle) * distance;


  // Don't grow offscreen
  if (
    x < 20 ||
    x > width - 20 ||
    y < 20 ||
    y > height - 20
  ) {
    return;
  }


  // Avoid putting new nodes directly on top of old ones
  let closestDistance = Infinity;


  for (let node of nodes) {

    let d =
      dist(
        x,
        y,
        node.x,
        node.y
      );

    closestDistance =
      min(
        closestDistance,
        d
      );
  }


  if (closestDistance < 30) {
    return;
  }


  // Create the new node
  let newNode =
    new SparkNode(
      x,
      y
    );


  nodes.push(newNode);


  // Connect the parent to the child
  source.neighbors.push(newNode);

  newNode.neighbors.push(source);


  // Connect to a couple nearby existing nodes
  let nearby =
    nodes
      .filter(n =>
        n !== newNode &&
        n !== source
      )
      .map(n => ({
        node: n,
        distance: dist(
          x,
          y,
          n.x,
          n.y
        )
      }))
      .filter(o =>
        o.distance <
        CONNECTION_RADIUS
      )
      .sort(
        (a, b) =>
          a.distance -
          b.distance
      );


  for (
    let i = 0;
    i < min(2, nearby.length);
    i++
  ) {

    newNode.neighbors.push(
      nearby[i].node
    );
  }


  // Send energy into the newly created territory
  pulses.push(

    new Pulse(
      source,
      newNode,
      energy * 0.85,
      hue
    )

  );
}

// ----------------------------------------------------
// QUADRATIC CURVE HELPER
// ----------------------------------------------------

function quadraticPoint(a, b, c, t) {

  let mt = 1 - t;

  return (
    mt * mt * a +
    2 * mt * t * b +
    t * t * c
  );

}

function compositionValue(x, y) {

  let value = 0;


  // Positive focal regions
  for (let p of focalPoints) {

    let d = dist(
      x,
      y,
      p.x,
      p.y
    );

    let influence =
      Math.exp(
        -sq(d / p.radius)
      );

    value +=
      influence *
      p.strength;
  }


  // Negative / quiet regions
  for (let p of voidPoints) {

    let d = dist(
      x,
      y,
      p.x,
      p.y
    );

    let influence =
      Math.exp(
        -sq(d / p.radius)
      );

    value -=
      influence *
      p.strength;
  }


  return constrain(
    value,
    0,
    1
  );
}


// ----------------------------------------------------
// INPUT
// ----------------------------------------------------

function mousePressed() {

  igniteAt(
    mouseX,
    mouseY
  );

}


function keyReleased() {

  if (key === " ") {

    background(225, 30, 4);

    buildNetwork();

    igniteRandomSpark();

  }


if (key === "s" || key === "S") {

    console.log("SAVE pressed:", millis());

    paintLayer.save(
      "TheSpark_" +
      currentPalette.name + "_" +
      year() + "-" +
      nf(month(), 2) + "-" +
      nf(day(), 2) + "_" +
      nf(hour(), 2) + "-" +
      nf(minute(), 2) + "-" +
      nf(second(), 2) +
      ".png"
    );
}

  if (key === "c" || key === "C") {

    paintLayer.clear();

  }

  if (key === " ") {

    igniteRandomSpark();

  }

  if (key === "r" || key === "R") {

    paintLayer.clear();

    buildComposition();
    buildNetwork();

    background(225, 30, 4);
  }

}


// ----------------------------------------------------
// RESIZE
// ----------------------------------------------------

function windowResized() { 

  resizeCanvas(
    windowWidth,
    windowHeight
  );

  buildNetwork();

  background(225, 30, 4);

}